...
def main():
    ...
    for line in BANNER.splitlines():
        head.append(line + "\n", style="bold red")
    head.append("USERNAME HUNTER V2 · 13 plateformes", style="dim")
    ...
    with Live(layout, screen=True, refresh_per_second=10):
        while len(results) < len(SITES):
            layout["header"].update(Panel(
                Align.center(Text.from_markup(f"[bold red]CHASSE : [white]{user}[/] [dim]||[/] [red]{progress}%")),
                border_style="red",
            ))
            ...
            for name, found in sorted(results):
                status = "[bold green][✓] TROUVÉ[/]" if found else "[dim red][x] ABSENT[/]"
                table.add_row(name.upper(), status)
            layout["body"].update(Panel(table, title="[bold white]SCAN LIVE", border_style="red"))
            layout["footer"].update(Panel(
                Align.center(Text.from_markup(f"[dim]VOID OSINT ENGINE - {current_status}[/]")),
                border_style="red",
            ))
            time.sleep(0.1)
    ...
    console.print(Align.center(Panel(
        Text.from_markup(f"[bold green]RAPPORT TERMINÉ : {user}"),
    ...
    for name, found in sorted(results):
        if found:
            ...
            final_table.add_row(name, "[bold green]ACTIF[/]", url)
